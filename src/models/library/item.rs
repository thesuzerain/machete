...
impl Rune {
    pub fn from_parts(s: &str, potency: i8) -> Self {
        match s {
            ...
            _ => Self::PropertyRune{property: s.to_string(), potency},
        }
    }

    pub fn to_parts(&self) -> (String, i8) {
        match self {
            Self::Resilient{potency: p} => ("Resilient".to_string(), *p),
            Self::ArmorPotency{potency: p} => ("Armor Potency".to_string(), *p), 
            Self::Striking{potency: p} => ("Striking".to_string(), *p),
            Self::WeaponPotency{potency: p} => ("Weapon Potency".to_string(), *p),
            Self::ShieldPotency{potency: p} => ("Shield Potency".to_string(), *p),
            Self::PropertyRune{property: s, potency: p} => (s.to_string(), *p),
        }
    }
    ...
    pub fn get_potency(&self) -> i8 {
        match self {
            ...
            Self::PropertyRune{property: _, potency: p} => *p,
        }
    }
}
